...
function recalculatePlayerStats() {
    ...
    for (const k in window.equipped) {
        ...
        if (i) {
            const upgMult = 1 + (i.upgradeLevel * 0.1); // +10% stat za level upg
            window.totalDmg += Math.floor((i.dmgBonus || 0) * upgMult);
            window.totalDps += Math.floor((i.dpsBonus || 0) * upgMult);
            window.playerMaxHp += Math.floor((i.hpBonus || 0) * upgMult);
    ...
    document.getElementById('advanced-stats').innerHTML = `<span class="text-accent text-bold">SUMA STATYSTYK:</span><br>Max HP: ${window.playerMaxHp} <br>DMG z kliknięcia: ${window.totalDmg} <br>Szansa na Krytyk: ${window.totalCrit}% <br>Kradzież Życia: ${window.totalLifesteal}%`;
}
...
function forgeItem() {
    ...
    if(window.gameData.gold >= costGold && window.gameData.iron >= costIron) {
        ...
        recalculatePlayerStats(); saveInventory(); inspectItem(inspectedItem); updateStatusUI();
        logMessage(`⚒️ Sukces Kuźni: ${inspectedItem.name} +${inspectedItem.upgradeLevel}!`);
    } else {
        logMessage("❌ Brak surowców do ulepszenia.");
    }
}

function saveInventory() { localStorage.setItem('de_inv_v3', JSON.stringify(window.inventory)); localStorage.setItem('de_eq_v3', JSON.stringify(window.equipped)); }
function loadInventory() {
    const sI = localStorage.getItem('de_inv_v3'); const sE = localStorage.getItem('de_eq_v3');
